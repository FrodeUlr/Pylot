...
mod tests {
    ...
    async fn test_create_venv_already_exists() {
        #[cfg(unix)]
        {
            ...
            list().await;
            let result = create("test_env", "3.11", vec!["numpy".to_string()], None, true).await;
            log::error!("Result: {:?}", result);
            assert!(result.is_ok());
            let result_exists =
                create("test_env", "3.11", vec!["numpy".to_string()], None, true).await;
    ...
    async fn test_create_venv_invalid_python() {
        #[cfg(unix)]
        {
            ...
            list().await;
            let result_pyerr =
                create("test_env", "0.1", vec!["numpy".to_string()], None, true).await;
    ...
    async fn test_create_venv_with_requirements() {
        #[cfg(unix)]
        {
            ...
            list().await;
            let result = create("test_env_req", "3.11", vec![], Some(&requirements), true).await;
            log::error!("Result: {:?}", result);
            assert!(result.is_ok());
            list().await;
